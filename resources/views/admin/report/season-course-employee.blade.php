        ...
        <div class="w-full">
            ...
            <div class="overflow-x-auto">
                <table class="w-full bg-white border border-gray-200 rounded-lg overflow-hidden">
                    ...
                    @foreach($data as $d)
                        <tr class="border-b">
                            <td class="border px-4 py-4">{{ \App\Models\User::whereId($d->created_by)->first()['user_login'] }}</td>
                            <td class="border px-4 py-4 text-center">Done</td>
                            <td class="border px-4 py-4 text-center">{{ \Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $d['date_created'])->format('M d, Y') }}</td>
                            <td class="border px-4 py-4 text-center"><a
                                    class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded"
                                    href="{{ route('season-employee-detail', ['formId' => $formId,'entryId' => $d->id, 'dateCreated'=> $d->date_created]) }}">Detail</a></td>
                        </tr>
                    @endforeach
                    </tbody>
                </table>
